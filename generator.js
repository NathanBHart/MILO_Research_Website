document.addEventListener("DOMContentLoaded", function () {
    ...
    if (techData["title"] !== "") {
        techTitle = `
            <h2 id="tech-title">${techData["title"]}</h2>
        `;
    } else {
        techTitle = `
            <h2 id="tech-title">Technology Title Missing</h2>
        `;
    }

    let techExecutiveSummary = "";
    if (techData["executive-summary"] !== "") {
        techExecutiveSummary = `
            <p id="executive-summary">${techData["executive-summary"]}</p>
        `;
    }
    ...
    if (techData["image-url"] !== "") {
        techFigure = `
                <figure class="tech-image"> 
                    <img src="${techData["image-url"]}" alt="${techData["alt-text"]}" loading="lazy">
                    <figcaption>${techData["image-caption"]}</figcaption>
                </figure>
            `;
    }
    ...
    if (techData["inventors"].length >= 1 && techData["inventors"][0] !== "") {
        techInventors = `
            <div id="inventors-block" class="mb-4">
                <h4 class="mb-2">Inventors:</h4>
                <p id="inventors">${techData["inventors"].join(", ")}</p>
            </div>
        `;
    }
    ...
    if (techData["patent-status"] !== "") {
        techPatentStatus = `
            <div id="patent-status-block" class="mb-4">
                <h4 class="mb-2">Patent Status:</h4>
                <p id="patent-status">${techData["patent-status"]}</p>
            </div>
        `;
    }
    ...
    if (techData["research-stage"] !== "") {
        techResearchStage = `
            <div id="research-stage-block" class="mb-4">
                <h4 class="mb-2">Stage of Research:</h4>
                <p id="research-stage">${techData["research-stage"]}</p>
            </div>
        `;
    }
    ...
    if (techData["contact-name"] !== "") {
        techContactName = `
            <p id="contact-name">${techData["contact-name"]}</p>
        `;
    }
    ...
    if (techData["contact-title"] !== "") {
        techContactTitle = `
            <p id="contact-title">${techData["contact-title"]}</p>
        `;
    }
    ...
    if (techData["contact-email"] !== "") {
        techContactEmail = `
            <a id="contact-email-btn" class="btn btn-primary btn-block btn-arrow mt-2" target="_blank" href="mailto:${techData["contact-email"]}" role="button">Email ${techData["contact-name"]}</a>
        `;
    }

    let techAlternativeContact = "";
    if (techData["alternative-contact"] !== "") {
        techAlternativeContact = `
            <a id="alternative-contact-btn" class="btn btn-primary btn-block btn-arrow mt-2" target="_blank" href="${techData["alternative-contact"]}" role="button">Contact ${techData["contact-name"]}</a>
        `;
    }
    ...
    if (techContactEmail !== "" || techAlternativeContact !== "") {
        techContactButtons = `
            <div id="contact-buttons" class="mb-4">
                ${techContactEmail}
                ${techAlternativeContact}
            </div>
        `;
    ...
    if (techData["contact-name"] !== "") {
        techContactBlock = `
            <div id="contact-block" class="mb-4">
    ...
    if (techData["abstract"] !== "") {
        techAbstract = `
            <div id="abstract-block" class="mb-4">
                <h4 class="mb-2">Abstract:</h4>
                <p id="abstract">${techData["abstract"]}</p>
            </div>
        `;
    }
    ...
    if (
        ...
        techApplications = `
            <div id="applications-block" class="mb-4">
                <h4 class="mb-2">Applications:</h4>
                <ul id="applications">${applicationsList}</ul>
            </div>
        `;
    }
    ...
    if (
        ...
        techAdvantages = `
            <div id="advantages-block" class="mb-4">
                <h4 class="mb-2">Advantages:</h4>
                <ul id="advantages">${advantagesList}</ul>
            </div>
        `;
    }
    ...
    if (
        ...
        techReferences = `
            <div id="references-block" class="mb-4">
                <h4 class="mb-2">References:</h4>
                <ul id="references">${referencesList}</ul>
            </div>
        `;
    }
                            ...
                            <div class="container">
                                <div class="technology">
                                    ...
                                    <div class="technology-body">
                                        <div class="row">
                                            <div class="col-lg-4 technology-sidebar">
                                                <div class="tech-id text-white">
                                                    <h4 class="mb-2 text-white" style="font-size: x-large;">Tech ID:</h4>
                                                    <p id="tech-id" class="text-white" style="font-size: larger;">${techId}</p>
                                                </div>
